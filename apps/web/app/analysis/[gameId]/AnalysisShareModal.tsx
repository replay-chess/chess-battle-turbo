...
function ShareActions({ url, shareText }: { url: string; shareText: string }) {
  ...
  const handleNativeShare = async () => {
    try {
      await navigator.share({ title: "Chess Battle", text: shareText, url });
    } catch {
      // User cancelled or share failed
    }
  };

  const encodedShareText = encodeURIComponent(shareText + " " + url);
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex gap-2">
        {hasNativeShare() && (
          <button onClick={handleNativeShare} className={buttonClass} style={{ fontFamily: "'Geist', sans-serif" }}>
        ...
        >
          ...
          <span style={{ fontFamily: "'Geist', sans-serif" }} className="text-xs tracking-[0.1em] font-semibold">
            WHATSAPP
          </span>
        </a>
        <a
      ...
      <button onClick={handleCopy} className={cn(buttonClass, "w-full flex-none")}>
        {copied ? (
          <div className="flex items-center gap-2">
            <Check className="w-3.5 h-3.5" />
            <span style={{ fontFamily: "'Geist', sans-serif" }} className="text-xs tracking-[0.1em] font-semibold">COPIED</span>
          </div>
        ) : (
          <div className="flex items-center gap-2">
            <Copy className="w-3.5 h-3.5" />
            <span style={{ fontFamily: "'Geist', sans-serif" }} className="text-xs tracking-[0.1em] font-semibold">COPY LINK</span>
          </div>
        )}
      </button>
    </div>
...
export function AnalysisShareModal({
  ...
  const handleCreateGame = async () => {
    ...
    try {
      ...
      const newInviteLink = `${origin}/join/${ref}?autojoin=true`;
      setCreatedGameRef(ref);
  ...
  return (
    <>
          ...
          >
            ...
            >
              ...
              <div className="mb-6">
                ...
                <ShareActions url={gameLink} shareText="Check out this chess game analysis!" />
              </div>
              ...
              {hasPosition && (
                <>
                  ...
                  <div className="mb-6">
                    ...
                    <ShareActions url={positionLink} shareText="Try playing this chess position!" />
                  </div>
                </>
              )}
      ...
      <ShareLinkModal
        isOpen={shareLinkModalOpen && !!inviteLink}
